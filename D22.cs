    ...
    internal class D22
    {
        ...
        public static void Run()
        {
            ...
            for (int toRemove = 0; toRemove < bricks.Count; toRemove++)
            {
                ...
                var newBricks = bricks[..toRemove].Concat(bricks[(toRemove + 1)..]).ToList();
                bool canRemove = true;
                for (int i = toRemove; i < newBricks.Count; i++)
                {
                    var (x1, y1, z1, x2, y2, z2) = newBricks[i];
                    if (originalZ + 1 != z1)
                        continue;
                    int minZ = 1;
                    for (int j = 0; j < i; j++)
                    {
                        var (x3, y3, z3, x4, y4, z4) = newBricks[j];
                        if (Insersect(((x1, y1), (x2, y2)), ((x3, y3), (x4, y4))))
                        {
                            minZ = Math.Max(minZ, z4 + 1);
                        }
